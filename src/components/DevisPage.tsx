...
const STATUT_TON: Record<StatutDevis, string> = {
    ...
    <section className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-stone-600">
            Fiches clients, devis PDF, envoi par mail (copie à Jonathan).
          </p>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        <input
          ...
          placeholder="Recherche client, n°, objet…"
          className="min-w-[12rem] flex-1 rounded border border-stone-300 px-3 py-2 text-sm"
        />
        <select
          value={filtre}
          onChange={(event) => setFiltre(event.target.value as StatutDevis | "tous")}
      ...
      {loading ? (
        ...
        <p className="text-sm text-stone-600">Aucun devis pour le moment.</p>
      ) : (
        <div className="overflow-hidden rounded-lg border border-stone-300 bg-white">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {visible.map((row) => {
                const ttc = totauxDevis(row.lignes, row.tva_pct).ttc;
                return (
                  <tr key={row.id} className="border-t border-stone-200">
                    ...
                    <td className="px-3 py-2">
                      <Link
                        href={`/devis/clients/${row.client_id}`}
                        className="underline"
                      >
                        {row.client_nom || "—"}
                      </Link>
                    </td>
                    <td className="px-3 py-2 text-stone-700">{row.objet || "—"}</td>
                    <td className="px-3 py-2">{formatIsoFr(row.date_devis)}</td>
                    <td className="px-3 py-2">
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs font-medium ${STATUT_TON[row.statut]}`}
                      >
