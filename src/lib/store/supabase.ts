...
let lastSnapshot: { at: number; data: PlanningSnapshot; gen: number } | null = null;
    ...
    inflightSnapshot = fetchSupabaseSnapshotOnce()
      .then((data) => {
        if (gen === snapshotGeneration) {
          lastSnapshot = { at: Date.now(), data, gen };
        }
        return data;
      })
      .finally(() => {
...
async function fetchSupabaseSnapshotOnce(): Promise<PlanningSnapshot> {
  ...
  const [signalementResult, receptions, demandes, sousTraitants] = extra;

  const signalementRows = signalementResult;
