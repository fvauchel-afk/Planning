...
const ASSIGN_REQUIRED: TypePhase[] = [
  "administratif",
  "fabrication",
  "pose",
  "livraison",
...
type OptionEditOptions = {
  ...
  employeAdministratifId?: string | null;
...
function preferredAssigneeForType(
  ...
  if (type === "administratif") return options.employeAdministratifId;
...
function cascadeAfterAssigneeChanges(
  ...
  for (const element of elements) {
    ...
    for (const type of ASSIGN_REQUIRED) {
      if (type === "administratif" && !options.avecAdministratif) continue;
...
function applyAssigneeEdits(
  ...
  for (const element of elements) {
    ...
    if (options.avecAdministratif) {
      assignExisting("administratif", options.employeAdministratifId, true);
    }
    assignExisting(
      "fabrication",
...
export function planChantierOptionEdits(
  ...
  const wantAdmin = Boolean(options.avecAdministratif);
  ...
  for (const element of elements) {
    ...
    if (wantAdmin) {
      ...
      const range = scheduleHoursOnAssignee(
        snapshot,
        options.employeAdministratifId || admin?.employe_id || null,
        earliestAvailableWorkDate(snapshot),
        adminHours,
      );
      if (admin && !deleteIds.includes(admin.id)) {
        ...
        if (shouldRecale) {
          patches.push({
            ...
            employe_id: pickEmployeeForPhase(
              ...
              range.start,
              range.end,
              options.employeAdministratifId || admin.employe_id,
            ),
        ...
        inserts.push({
          ...
          employe_id: pickEmployeeForPhase(
            ...
            range.start,
            range.end,
            options.employeAdministratifId,
          ),
          statut: "a_faire",
          urgent: false,
          heures_supplementaires_par_jour: 0,
